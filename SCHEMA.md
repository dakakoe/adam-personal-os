...
**`raw.linkedin_profile_capture`**: one row per LinkedIn vanity, written by the browser extension (`browser_ext/linkedin_capture`) via `POST /api/capture/linkedin`. Superset of `raw.linkedin_connection`'s columns — the CSV export's first/last/company/position plus what only the page shows (headline, location, about, avatar, connection degree, and the contact-info overlay's emails/phones/websites). Unique on `vanity`; re-capturing refreshes in place, because a profile changes and the newest read wins. LinkedIn has no public API and blocks server-side fetching, so this is the only live path in.
