...
IdentitySource = Literal["telegram", "email", "linkedin", "phone", "whatsapp", "x",
                         "instagram", "github", "website", "telegram_handle"]
...
class PersonRow(BaseModel):
    ...
    # Inline so the list can show and edit tags without a call per row.
    circles: list["PersonCircleRef"] = Field(default_factory=list)
...
class PersonCircleRef(BaseModel):
    """A circle as seen from a contact (no member_count — that's the admin view)."""
    id: str
    key: str
    label: str
    priority: int = 100
...
class ContactCircle(BaseModel):
    """A curated relationship group (family, investors, friends…). `priority`
    both sorts and ranks — a contact in several circles inherits the strongest
    (lowest number). `cadence_days` is how often you mean to stay in touch;
    null means the circle is a pure label."""
    id: str
    key: str
    label: str
    priority: int = 100
...
class ContactCircleIn(BaseModel):
...
class ContactCirclePatch(BaseModel):
    key: str | None = None
    label: str | None = None
...
class PersonCirclesIn(BaseModel):
    circle_ids: list[str] = Field(default_factory=list)


class CircleDueRow(BaseModel):
    """A contact you're overdue to speak to, per their circle's cadence."""
    person_id: str
    display_name: str
    circle_label: str | None = None
    priority: int = 100
    cadence_days: int | None = None
    last_interaction_at: datetime | None = None
...
class FollowupNote(BaseModel):
    """One update on a follow-up: what was discussed, what changed.

    Distinct from the follow-up's `topic`, which is what you MEANT to discuss,
    fixed when you planned it. These accumulate as things actually happen."""
    id: str
    body: str
    created_at: datetime
...
class FollowupRow(BaseModel):
    """A conversation you owe someone: person + when + what about.

    `connected` is the answer to "did it happen", combining a manual tick with
    anything seen on an ingested channel. `connected_source` says which told
    us — 'auto' (read off canonical.interaction) or 'manual' (you ticked it,
    because it happened somewhere we can't see)."""
    id: str
    person_id: str
    display_name: str
    due_date: date | None = None        # null = someday, no particular day
    due_time: time | None = None        # null = all-day, "some point that day"
    topic: str | None = None
    # How much it matters. Drives the order the page shows and the order
    # distribute hands out the coming days.
    priority: Literal["low", "mid", "high"] = "mid"
    status: Literal["open", "connected", "cancelled"]
    # Who the person is, so the row can be acted on without opening them:
    # their current company and one line of role/context from the profile.
    ...
    connected_via: str | None = None          # 'telegram_text', 'gmail', 'manual', …
    connected_source: Literal["auto", "manual"] | None = None
    notes: list[FollowupNote] = Field(default_factory=list)   # newest first
    created_at: datetime


class FollowupCreate(BaseModel):
    person_id: str
    due_date: date | None = None        # omit for a someday follow-up
    due_time: time | None = None        # omit for an all-day follow-up
    topic: str | None = Field(None, max_length=2000)
    priority: Literal["low", "mid", "high"] = "mid"


class FollowupBulkIds(BaseModel):
    """The selection a bulk action applies to. Capped: the list page fetches
    200 at a time, so anything past that is a client bug, not a big selection."""
    ids: list[str] = Field(..., min_length=1, max_length=500)


class FollowupBulkPatch(FollowupBulkIds):
    """One edit, applied to every selected follow-up.

    Deliberately narrower than FollowupPatch: `topic` is missing because it's
    the one field that is per-person by nature — stamping "Whats' up" across
    fifty follow-ups destroys the only thing that made them distinguishable."""
    due_date: date | None = None        # null clears it → someday pile
    due_time: time | None = None        # null clears it → all-day
    # Priority IS bulk-safe, unlike topic: "these eighty matter less" is a
    # judgement about the whole selection, not about each person in it.
    priority: Literal["low", "mid", "high"] | None = None
    connected: bool | None = None
    status: Literal["open", "connected", "cancelled"] | None = None


class FollowupBulkResult(BaseModel):
    """How many rows actually changed. Lower than you selected means some were
    already deleted or settled elsewhere — worth saying out loud."""
    changed: int


class FollowupSlot(BaseModel):
    """The recommended date for a new follow-up — the next day with capacity."""
    date: date


class FollowupDistributeResult(BaseModel):
    """Outcome of spreading the overdue backlog forward: how many follow-ups
    moved, across how many days."""
    moved: int
    days: int


class FollowupPatch(BaseModel):
    """Every field optional: reschedule, re-topic, tick, or cancel.

    `connected: true` marks it done by hand — for a call or a coffee, the
    cases the automatic rule can't see. `connected: false` reopens it."""
    # Both dates are explicitly nullable: null clears them. A cleared date
    # moves the follow-up to the someday pile.
    due_date: date | None = None
    # Explicitly nullable: sending null clears the time, turning a timed
    # follow-up back into an all-day one. Omitting it leaves the time alone.
    due_time: time | None = None
    topic: str | None = Field(None, max_length=2000)
    priority: Literal["low", "mid", "high"] | None = None
    connected: bool | None = None
    connected_via: str | None = Field(None, max_length=64)
    status: Literal["open", "connected", "cancelled"] | None = None
...
class IdentityRow(BaseModel):
    ...
    source: str
    source_id: str
    # Telegram only: the @handle for THIS account's numeric id. Null for other
    # sources, and for a telegram id we've never seen a profile for.
    username: str | None = None
    evidence: dict[str, Any] | None = None
    created_at: datetime


class NameDismiss(BaseModel):
    """One candidate display name to stop offering for a person."""
...
class PersonMessage(BaseModel):
    """One interaction in a contact's history.

    `body` can be empty — a photo or voice note with no caption is still a
    message — and `truncated` says the stored body was longer than the page
    serves, so nothing silently pretends to be the whole thing."""
    id: str
...
class PersonChannelCount(BaseModel):
    channel: str
    count: int


class PersonMessagesPage(BaseModel):
    """A page of history plus what the filter needs to describe itself."""
    total: int                                   # matching the current filters
    channels: list[PersonChannelCount]           # ignoring the channel filter
...
class PersonDetail(BaseModel):
    ...
    location: str | None = None    # stored, editable; seeded from LinkedIn
...
class LocationIn(BaseModel):
    """Set or clear a person's location. Personal, not job — "Lisbon", "Remote".
...
class LinkedInExperience(BaseModel):
    """One row of the profile's Experience/Education list, as rendered.

    A grouped entry (several roles at one employer) arrives already flattened
    into one row per role, each carrying the shared company — so the history
    reads as a plain list here and in raw.linkedin_profile_capture.experience.
    """
    title: str | None = None
    company: str | None = None
    employment_type: str | None = Field(None, max_length=64)   # Full-time, Contract, …
    date_range: str | None = Field(None, max_length=120)       # "Mar 2026 - Present"
    duration: str | None = Field(None, max_length=64)          # "6 mos"
    location: str | None = None
    description: str | None = Field(None, max_length=4000)


class LinkedInCapture(BaseModel):
    """A LinkedIn profile read off the page by the browser extension.

    Everything except `vanity` is optional: LinkedIn's markup shifts, the
    contact-info overlay may be closed, and a half-scraped profile is still
    worth more than nothing. Field names mirror raw.linkedin_profile_capture.
    """
    vanity: str = Field(..., min_length=1, max_length=200)
    ...
    headline: str | None = Field(None, max_length=2000)
    location: str | None = Field(None, max_length=500)
    about: str | None = Field(None, max_length=20000)
    current_title: str | None = Field(None, max_length=500)
    current_company: str | None = Field(None, max_length=500)
    company_url: str | None = Field(None, max_length=1000)
    avatar_url: str | None = Field(None, max_length=2000)
    connection_degree: str | None = Field(None, max_length=32)
    ...
    birthday: str | None = Field(None, max_length=100)
    experience: list[LinkedInExperience] = Field(default_factory=list, max_length=25)
    education: list[LinkedInExperience] = Field(default_factory=list, max_length=25)
    note: str | None = Field(None, max_length=4000)   # free-text, appended to person notes


class LinkedInCaptureResult(BaseModel):
    """What the capture did, so the extension can say it plainly."""
    person_id: str
    display_name: str
    vanity: str
    person_created: bool
    identity_created: bool
    name_upgraded: bool = False
    emails_linked: list[str] = Field(default_factory=list)
    emails_conflicted: list[str] = Field(default_factory=list)  # already another person's
    phones_recorded: int = 0
    signals_recorded: int = 0
    note_added: bool = False


class LinkedInCaptureLookup(BaseModel):
    """GET side: is this vanity already someone we know?"""
    vanity: str
    known: bool
    person_id: str | None = None
...
class AttendeeDismissIn(BaseModel):
    email: str


class PhoneNumberFixItem(BaseModel):
    digits: str = Field(..., pattern=r"^[0-9]{1,20}$")
    action: Literal["set", "ignore"]
    # For "set": the number in international form; '+' and spacing allowed.
    international: str | None = None


class PhoneNumberFixIn(BaseModel):
    items: list[PhoneNumberFixItem] = Field(..., min_length=1, max_length=2000)


class AttendeeLinkIn(BaseModel):
    email: str
    # Omit to create a new contact from the attendee's name/address.
    person_id: str | None = None
    name: str | None = None
