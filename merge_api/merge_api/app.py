...
def create_app() -> FastAPI:
    ...
    async def list_persons_route(
        ...
        rows = await queries.list_persons(
            pool, q=q, company_id=company_id, limit=limit, offset=offset, viewer=viewer
        )
        return rows
    ...
    async def count_persons_route(
        ...
        filters. Registered BEFORE /api/persons/{person_id} so the literal
        'count' path segment isn't shadowed by the UUID parameter route."""
        pool = request.app.state.pool
        viewer = await auth.resolve_viewer(request)
        return {"count": await queries.count_persons(pool, q=q, company_id=company_id, viewer=viewer)}
