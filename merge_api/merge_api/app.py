...
from . import auth, db, extraction, finance_categorize, gcal_write, gmail_fetch, gmail_send, oauth_reconnect, queries, rspamd, setup_flow, vcard
from .models import (
    AttendeeDismissIn,
    AttendeeLinkIn,
    ...
    FollowupBulkIds, FollowupBulkPatch, FollowupBulkResult, FollowupDistributeResult, FollowupSlot,
    LinkedInCapture, LinkedInCaptureLookup, LinkedInCaptureResult,
    OpportunityStageIn, PersonDetail, PersonMessagesPage, PersonRow, PersonSensitivity, ContactSharing, ProjectCreate,
    ProspectRow,
    ProjectDetail, ProjectMemberIn, ProjectPatch, ProjectRow, RenameIn, BirthdayIn, LocationIn,
    NameDismiss,
    ...
    StreamRow, StreamCreateIn, StreamPatchIn,
...
def create_app() -> FastAPI:
    ...
    HEALTH_UNITS = [
        ...
        ("memory-whatsapp.service", "WhatsApp ingest", "always_on"),
    ...
    async def sources_status(request: Request, _: None = Depends(auth.verify)):
        ...
        SOURCES = [
            ...
            # Not OAuth, so health and the reconnect reason both come from
            # memory.source_status — the bridge writes 'whatsapp' there, and
            # this key must match it or the card shows a permanent unknown.
            # Always-on, so deliberately absent from SYNCABLE_UNITS: there is
            # no "sync now" for a socket that is already listening.
            ("whatsapp", "WhatsApp", "memory-whatsapp.service", "always_on",
             "SELECT count(*) FROM raw.whatsapp_message",
             "SELECT max(ingested_at) FROM raw.whatsapp_message", "messages", None, None, None),
    ...
    @app.post("/api/setup/iphone-contacts/upload")
    async def setup_iphone_contacts_upload(request: Request, file: UploadFile = File(...),
                                           role: str = Depends(auth.verify)):
        """Upload the phone book exported from an iPhone (.vcf) and store every
        card in raw.iphone_contact; the normalizer turns them into people on its
        next run (every 5 minutes).

        Parsed here, not in a separate importer app like LinkedIn's: a vCard
        needs nothing beyond the standard library. The file streams to disk
        under the upload cap, is read line by line with photos dropped as they
        go, and is deleted afterwards — it is the entire phone book."""
        if role != "admin":
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="admin only")
        if not setup_flow.valid_vcf_name(file.filename):
            raise HTTPException(status.HTTP_400_BAD_REQUEST,
                                detail="upload the .vcf file exported from Contacts")
        os.makedirs(_IMPORTS_DIR, exist_ok=True)
        dest = os.path.join(_IMPORTS_DIR, f"iphone-contacts-{int(datetime.now().timestamp())}.vcf")
        country = request.app.state.cfg.iphone_default_country_code
        try:
            written = 0
            with open(dest, "wb") as f:
                while chunk := await file.read(1 << 20):
            ...
            def _parse() -> list[vcard.Contact]:
                with open(dest, encoding="utf-8", errors="replace") as fh:
                    return vcard.parse_vcards(fh, default_country_code=country)

            contacts = await asyncio.to_thread(_parse)
        finally:
            if os.path.exists(dest):
                os.unlink(dest)
        if not contacts:
            raise HTTPException(status.HTTP_400_BAD_REQUEST,
                                detail="no contacts found — is this a vCard export from Contacts?")
        counts = await queries.upsert_iphone_contacts(request.app.state.pool, contacts)
        counts.update(vcard.summarize(contacts))
        log.info("iphone contacts upload: %s", counts)
        return {"ok": True, "counts": counts}

    @app.get("/api/contacts/numbers/unplaced")
    async def unplaced_numbers_route(request: Request, role: str = Depends(auth.verify)):
        """Phone-book numbers that can't be matched to anyone yet: written
        without a country code, and not fixed or deleted by the user."""
        if role != "admin":
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="admin only")
        return await queries.list_unplaced_numbers(
            request.app.state.pool, request.app.state.cfg.iphone_default_country_code)

    @app.post("/api/contacts/numbers/fix")
    async def fix_numbers_route(request: Request, body: PhoneNumberFixIn,
                                role: str = Depends(auth.verify)):
        """Fix ("set") or delete ("ignore") numbers. Accepts many at once, so a
        whole group of suggestions applies in one request."""
        if role != "admin":
            raise HTTPException(status.HTTP_403_FORBIDDEN, detail="admin only")
        items: list[dict] = []
        for it in body.items:
            if it.action == "ignore":
                items.append({"digits": it.digits, "action": "ignore"})
                continue
            number = vcard.parse_international(it.international)
            if not number:
                raise HTTPException(
                    status.HTTP_400_BAD_REQUEST,
                    detail=f"{it.international or '(empty)'} is not an international number — "
                           "start with + and the country code")
            items.append({"digits": it.digits, "action": "set", "international": number})
        stats = await queries.apply_phone_number_fixes(request.app.state.pool, items)
        log.info("phone number fixes: %s", stats)
        return stats
    ...
    async def list_persons_route(
        ...
        ?source= takes an identity source ('telegram', 'email') — everyone you
        have an account of that kind for. ?sort= is one of
        interactions (default) | recent | oldest, the last two by when you last
        spoke; an unknown value falls back to the default."""
        pool = request.app.state.pool
        limit = max(1, min(limit, 200))
        viewer = await auth.resolve_viewer(request)   # member → shared + own contacts only
        rows = await queries.list_persons(
            pool, q=q, company_id=company_id, circle=circle, source=source,
            sort=sort, limit=limit, offset=offset, viewer=viewer,
        )
        return rows
    ...
    async def count_persons_route(
        ...
        ?circle= + ?source= filters. Registered BEFORE /api/persons/{person_id}
        so the literal 'count' path segment isn't shadowed by the UUID route."""
        pool = request.app.state.pool
        viewer = await auth.resolve_viewer(request)
        return {"count": await queries.count_persons(
            pool, q=q, company_id=company_id, circle=circle, source=source, viewer=viewer)}

    @app.get("/api/persons/sources")
    async def list_person_sources_route(request: Request, _: None = Depends(auth.verify)):
        """Identity sources with people counts — the options for the People
        list's source filter. Registered BEFORE /api/persons/{person_id} for the
        same reason as 'count'."""
        viewer = await auth.resolve_viewer(request)
        return await queries.list_person_sources(request.app.state.pool, viewer=viewer)
    ...
    @app.get("/api/followups/next-slot", response_model=FollowupSlot)
    async def next_followup_slot_route(request: Request, _: None = Depends(auth.verify)):
        """The day a new follow-up should default to: the next one under the
        per-day cap. Lets the contact-page 'Follow up' button recommend a date
        that isn't already full, without the page knowing the global load."""
        return await queries.next_followup_slot(request.app.state.pool)
    ...
    # Declared before /api/followups/{followup_id}, like the bulk routes, so
    # 'distribute' isn't matched as a followup id.
    @app.post("/api/followups/distribute", response_model=FollowupDistributeResult)
    async def distribute_followups_route(request: Request, _: None = Depends(auth.verify)):
        """Spread the owed backlog across the coming days, capped per day, so a
        wall of eighty becomes a workable few-a-day. High-priority follow-ups
        get the earliest days and push lower-priority ones back. Today's plan
        and anything with a clock time are left alone. One transaction."""
        return await queries.distribute_followups(request.app.state.pool)
    ...
    @app.get("/api/persons/{person_id}/messages", response_model=PersonMessagesPage)
    async def person_messages_route(
        person_id: str, request: Request,
        limit: int = 50, offset: int = 0,
        channel: str | None = None, q: str | None = None,
        _: None = Depends(auth.verify),
    ):
        """A contact's whole history, paged — the rest of what the contact
        page shows ten of. Same visibility gate as the contact itself."""
        pool = request.app.state.pool
        viewer = await auth.resolve_viewer(request)
        if not await queries.member_can_see(pool, viewer, person_id, table="canonical.person"):
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="person not found")
        return await queries.person_messages(
            pool, person_id, channel=channel, q=q, limit=limit, offset=offset)
    ...
    @app.post("/api/persons/{person_id}/name-suggestions/dismiss", status_code=204)
    async def dismiss_name_suggestion_route(
        person_id: str, body: NameDismiss, request: Request,
        _: None = Depends(auth.verify),
    ):
        """Stop offering one candidate name for this person.

        The suggestions are re-derived from the raw tables on every request, so
        without somewhere to record a 'no' the banner asks forever — most
        visibly after a merge, where the contact carries a second Telegram
        profile whose names are perfectly real and simply not wanted."""
        ok = await queries.dismiss_name_suggestion(
            request.app.state.pool, person_id, body.name)
        if not ok:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="person not found")
        return None
    ...
    @app.put("/api/persons/{person_id}/location")
    async def set_person_location_route(
        person_id: str, body: LocationIn, request: Request,
        _: None = Depends(auth.verify),
    ):
        """Set or clear a person's (personal) location. Null/empty clears it.
        Hand edits win over the backfill, which only fills a NULL."""
        loc = (body.location or "").strip() or None
        ok = await queries.set_person_location(request.app.state.pool, person_id, loc)
        if not ok:
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="person not found (or merged away)")
    ...
    async def ingest_granola_route(
        ...
        # Who "the user" is in THIS meeting. Granola lists the owner among the
        # attendees like anyone else, so the attendee whose email is one of
        # your own inboxes names you exactly as the recap will — which the
        # configured label cannot: the label is a short name, the recap says
        # the full one, and only one of the two was ever recognised.
        own_emails = await queries.own_email_addresses(pool)
        meeting_self_names = queries.self_names_from_attendees(
            attendees, own_emails, cfg.self_names)
        # Name the user for the prompt using the spelling this meeting uses.
        self_label = next(
            (" ".join((a.get("name") or "").split())
             for a in attendees
             if isinstance(a, dict)
             and (a.get("email") or "").strip().lower() in own_emails
        ...
        self_names = meeting_self_names

        async def _resolve(name: str | None):
            if not name or not name.strip():
                return None, None, "medium"
            # You are not one of your own contacts. A recap names you exactly
            # the way it names everyone else — "the user and Dana" yields action
            # items whose person is "the user" — and the resolver prefix-matches,
            # so without this the task is filed against a contact who merely
            # shares the first name. Treated as no person at all: a task with
            # yourself is just a task, and you are never a counterparty.
            if queries.is_self_name(name, self_names):
                return None, None, "medium"
        ...
        for pm in extracted.get("people_mentioned") or []:
            ...
            if not _is_substantive_reason(reason):
                continue
            # The prompt already says to leave the user out; this is the
            # backstop for when the model includes you anyway.
            if queries.is_self_name(name, self_names):
                continue
            ...
            if parsed.get("project_slug"):
                for p in projects:
                    if p["slug"] == parsed["project_slug"]:
                        ...
                        break
            # Same guard as the meeting path: "remind me to send the user the
            # deck" is about you, not the contact who shares the name.
            if nm and not queries.is_self_name(nm, app.state.cfg.self_names):
    ...
    # --- streams (deal tags, self-serve editor) -------------------------

    @app.get("/api/streams", response_model=list[StreamRow])
    async def list_streams_route(request: Request, _: None = Depends(auth.verify)):
        """Every stream with its live deal count, alphabetical."""
        return await queries.list_streams(request.app.state.pool)

    @app.post("/api/streams", response_model=StreamRow)
    async def create_stream_route(
        body: StreamCreateIn, request: Request, _: None = Depends(auth.verify),
    ):
        row = await queries.create_stream(request.app.state.pool, body.name)
        if row is None:
            raise HTTPException(status.HTTP_409_CONFLICT,
                                detail="that stream already exists")
        return row

    @app.patch("/api/streams/{name}", response_model=StreamRow)
    async def rename_stream_route(
        name: str, body: StreamPatchIn, request: Request,
        _: None = Depends(auth.verify),
    ):
        """Renames across every deal carrying the tag. Renaming onto an existing
        stream merges the two, which is how near-duplicates get cleaned up."""
        row = await queries.rename_stream(request.app.state.pool, name, body.name)
        if row == "bad_name":
            raise HTTPException(status.HTTP_400_BAD_REQUEST, detail="name required")
        return row

    @app.delete("/api/streams/{name}", status_code=204)
    async def delete_stream_route(
        name: str, request: Request, force: bool = False,
        _: None = Depends(auth.verify),
    ):
        """`force=true` also strips the tag from the deals still carrying it."""
        err = await queries.delete_stream(request.app.state.pool, name, force=force)
        if err == "not_found":
            raise HTTPException(status.HTTP_404_NOT_FOUND, detail="stream not found")
        if err == "in_use":
            raise HTTPException(status.HTTP_409_CONFLICT,
                                detail="stream is on deals — remove it there first")
    ...
    # --- meetings: attendees we could not resolve -------------------------
    # Named routes before any /{id} route, per the usual ordering rule.

    @app.get("/api/meetings/unknown-attendees")
    async def unknown_attendees_route(request: Request, limit: int = 25,
                                      _: None = Depends(auth.verify)):
        return await queries.unknown_attendees(
            request.app.state.pool, limit=max(1, min(limit, 100)))

    @app.get("/api/meetings/unknown-attendees/count")
    async def unknown_attendees_count_route(request: Request,
                                            _: None = Depends(auth.verify)):
        return {"count": await queries.count_unknown_attendees(request.app.state.pool)}

    @app.post("/api/meetings/unknown-attendees/dismiss", status_code=204)
    async def dismiss_attendee_route(request: Request, body: AttendeeDismissIn,
                                     _: None = Depends(auth.verify)):
        await queries.dismiss_attendee(request.app.state.pool, body.email)

    @app.post("/api/meetings/unknown-attendees/link")
    async def link_attendee_route(request: Request, body: AttendeeLinkIn,
                                  _: None = Depends(auth.verify)):
        """Attach the address to a person — an existing one, or a new contact.

        Writes the email identity too, so the next meeting resolves itself and
        mail from the same address lands on the same person.
        """
        pool = request.app.state.pool
        person_id = body.person_id
        if not person_id:
            person_id = await queries.create_person_for_attendee(pool, body.email, body.name)
        return await queries.link_attendee(pool, body.email, person_id)
