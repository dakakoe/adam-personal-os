...
def is_synthetic_display_name(name: str) -> bool:
    ...
    # "Will @iwillpat | Third Earth Capital", "Dan | Deel", "john@example.com".
...
async def create_followup(
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            """
            INSERT INTO memory.followup (person_id, due_date, due_time, topic)
            VALUES ($1::uuid, $2, $3, $4)
            RETURNING id::text
            """,
...
def plan_distribution(
    ...
    `overdue_ids` MUST already be in priority order (most overdue first) — they
    get the earliest open slots. `existing_load` is date-string → count of
    follow-ups already sitting on that future day, so a day that starts at 6
    only takes 4 more before spilling. Returns {followup_id: 'YYYY-MM-DD'}.
    """
    cap = max(1, cap)
    ...
    for fid in overdue_ids:
        # Advance past any day already at the cap. Monotonic: because ids are
        # priority-ordered, once a day fills we never need to look back at it.
...
async def distribute_followups(pool: asyncpg.Pool, cap: int = FOLLOWUP_DISTRIBUTE_CAP) -> dict:
    """Re-date every overdue owed follow-up onto the coming days, capped per day.

    Owed = open, not manually ticked, and not auto-settled by a message since it
    was created (the same NOT-EXISTS the list uses, so we never move a card the
    user already sees as settled). Overdue = due before today in the planner tz.
    Days are counted from tomorrow on: today's plan is left as it stands.
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            """
            SELECT f.id::text AS id, f.due_date, f.due_time
              FROM memory.followup f
             WHERE f.deleted_at IS NULL
               AND f.status = 'open'
        ...
        # Overdue → to move, oldest first so the most overdue gets tomorrow.
        overdue = sorted(
            (r for r in rows if r["due_date"] < today),
            key=lambda r: (r["due_date"], r["due_time"] or time.min),
        )
        # Existing future load (tomorrow on): what each day already carries.
        load: dict[str, int] = {}
        for r in rows:
        ...
        plan = plan_distribution([r["id"] for r in overdue], load, tomorrow, cap)

        async with conn.transaction():
            for fid, ds in plan.items():
                await conn.execute(
                    "UPDATE memory.followup SET due_date = $2::date "
                    " WHERE id = $1::uuid AND deleted_at IS NULL",
                    fid, date.fromisoformat(ds))

    return {"moved": len(plan), "days": len(set(plan.values()))}
