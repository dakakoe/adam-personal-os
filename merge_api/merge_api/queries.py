     ...
     AND ($3::text IS NULL
          ...
          OR ($3::text <> '__none__' AND EXISTS (
                SELECT 1 FROM memory.contact_circle_member m
                 ...
                 WHERE m.person_id = p.id AND c.key = $3::text)))/*VIS*/
)
SELECT
  b.*,
  (SELECT u.username
...
async def count_persons(pool: asyncpg.Pool, *, q: str | None, company_id: str | None = None,
                        circle: str | None = None, viewer: dict | None = None) -> int:
    """Total live persons matching the optional name-substring + company +
    circle filters (scoped to what `viewer` may see). Powers the
    'Showing N–M of TOTAL' header on the persons list."""
    args: list = [q, company_id, circle]
...
async def list_persons(
    ...
    circle: str | None = None, viewer: dict | None = None,
) -> list[dict]:
    """`circle` is a circle KEY ('family'), not an id — it's what the URL
    carries, so the filter survives being shared or bookmarked."""
    # scope to what the viewer may see (owner/None → all; member → shared + own).
    # the member-id bind lands at $6, after q/company_id/circle/limit/offset.
    args: list = [q, company_id, circle, limit, offset]
