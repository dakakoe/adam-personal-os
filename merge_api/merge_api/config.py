...
class Config:
    ...
    self_label: str
    # Names that mean YOU when they turn up in a meeting recap or a capture.
    # A recap of "the user and Dana" says "the user will send the deck", and the
    # name resolver prefix-matched that to a CONTACT who shares the first name
    # — so a task you owned was filed as a task with a stranger.
    #
    # Seeded from self_label, plus MERGE_API_SELF_NAMES for the spellings a
    # transcript actually produces (transliterations, diminutives):
    #     MERGE_API_SELF_NAMES='Sergey,Serge,Серёжа'
    # Matching is whole-string, so a fuller name still reaches the real
    # contact: "the user" is you, "the user Chan" is him.
    ...
    # is unaffected).
    mcp_embed_url: str
    # Country code for phone-book numbers written without one ("081 234 5678").
    # Unset = such numbers are stored but never matched to anyone.
    iphone_default_country_code: str | None
    mcp_bearer: str | None


# Placeholder labels ship as defaults and must never be treated as your name —
# "the user" appearing in a recap is prose, not an identity.
_LABEL_PLACEHOLDERS = {"the user", "the user (me)", "me", "user", "owner",
                       "the account owner", "the owner"}


def _self_names(self_label: str, extra: str) -> frozenset[str]:
    """The set of names that mean YOU, lowercased.

    Takes self_label whole ("Alex Rivera") and, when it is a plain
    personal name, its first token too — a transcript almost always writes the
    first name alone. Multi-word entries in `extra` are kept whole so an alias
    with a space still works.
    """
    out: set[str] = set()

    def add(raw: str) -> None:
        n = " ".join((raw or "").split()).strip().lower()
        if len(n) >= 2 and n not in _LABEL_PLACEHOLDERS:
            out.add(n)

    label = " ".join((self_label or "").split()).strip()
    if label.lower() not in _LABEL_PLACEHOLDERS:
        add(label)
        parts = label.split()
        # Only for a name-shaped label: a first token off "the account owner"
        # would blocklist the word "the".
...
def load() -> Config:
    return Config(
        ...
        self_names=_self_names(
            os.environ.get("MERGE_API_SELF_LABEL", "the user"),
            os.environ.get("MERGE_API_SELF_NAMES", ""),
        ),
        ...
        iphone_default_country_code=os.environ.get("IPHONE_CONTACTS_DEFAULT_COUNTRY_CODE") or None,
