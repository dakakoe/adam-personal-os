...
# An iOS export is named after its first contact ("Dana Lee and 250 others.vcf"),
# so names, spaces, commas and apostrophes are all normal here.
_VCF_NAME_RE = re.compile(r"^[\w][\w .,()'&+-]{0,200}\.vcf$", re.IGNORECASE)


def valid_vcf_name(filename: str | None) -> bool:
    """Basename sanity for an iPhone contacts export: .vcf, no path parts."""
    if not filename:
        return False
    if "/" in filename or "\\" in filename or ".." in filename:
        return False
    return _VCF_NAME_RE.match(filename) is not None
