...
class TestNormalizeVanity:
    ...
    @pytest.mark.parametrize("value", [
        "https://www.linkedin.com/in/nicolo-stöhr-692365160/",
        "nicolo-stöhr-692365160",
        "https://www.linkedin.com/in/nicolo-st%C3%B6hr-692365160/",
        "nicolo-st%C3%B6hr-692365160",
    ])
    def test_accented_vanity_encodes_instead_of_failing(self, value):
        """The address bar and the extension both hand back the DECODED
        spelling, so a capture of any accented name used to be rejected
        outright ('unparseable linkedin vanity').

        Encoding rather than widening the allowed characters is the point:
        both spellings have to land on the one form already stored for the
        people imported from CSV, or a capture forks a duplicate identity."""
        assert q.normalize_linkedin_vanity(value) == "nicolo-st%c3%b6hr-692365160"
