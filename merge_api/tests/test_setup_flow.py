...
def test_valid_vcf_name_accepts_ios_export_names():
    from merge_api.setup_flow import valid_vcf_name
    assert valid_vcf_name("Contacts.vcf")
    assert valid_vcf_name("Dana Lee and 250 others.vcf")
    assert valid_vcf_name("O'Brien, Sam.VCF")
    assert not valid_vcf_name("contacts.csv")
    assert not valid_vcf_name("../contacts.vcf")
    assert not valid_vcf_name("dir/contacts.vcf")
    assert not valid_vcf_name(None)
