...
/** Which bucket a follow-up belongs in. The list is grouped rather than one
 *  long ordering because "overdue" and "next week" call for different
 *  attention, and a flat list makes you re-derive that from every date. */
export type Bucket = "overdue" | "today" | "tomorrow" | "week" | "later" | "someday" | "settled";
...
export const BUCKET_ORDER: Bucket[] = [
  "overdue", "today", "tomorrow", "week", "later", "someday", "settled",
];
...
export function bucketOf(dueDate: string | null, dueTime: string | null): Bucket {
  ...
  if (off === 0) {
    ...
    return dueTime && daysLate(dueDate, dueTime) > 0 ? "overdue" : "today";
  }
