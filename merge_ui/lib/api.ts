...
export type IdentityRow = {
  ...
  source: string;
  source_id: string;
  /** Telegram only: the @handle belonging to THIS account's numeric id. */
...
/** A deal tag, with how many live deals carry it. */
...
/** An identity source with how many people carry one — People list filter. */
export type PersonSource = { source: string; people: number };

/** A phone-book number that can't be matched: written without a country code. */
export type UnplacedNumber = {
  digits: string;
  written: string;
  cards: { uid: string; name: string | null }[];
  suggestion: { action: "set" | "ignore"; international?: string; label: string } | null;
};
...
export type FollowupPriority = "low" | "mid" | "high";

export type FollowupRow = {
  id: string;
  person_id: string;
  display_name: string;
  due_date: string | null;          // "2026-08-12", or null for a someday one
  due_time: string | null;          // "15:00:00", or null for an all-day one
  topic: string | null;
  priority: FollowupPriority;       // orders the list and the distribution
  status: "open" | "connected" | "cancelled";
  // Who they are, so a row can be judged without opening the contact.
  company: string | null;
  company_id: string | null;        // null when the name came from LinkedIn only
  context: string | null;           // one line of role / what they do
  last_interaction_at: string | null;
...
/** How the People list is ordered. 'interactions' is the default — the
 *  people you deal with most, first — and the other two answer "who have I
 *  spoken to lately" and "who have I let go cold". */
export type PersonSort = "interactions" | "recent" | "oldest";
...
export type PersonMessage = {
  ...
  channel: string;
  body: string | null;        // empty for a photo/voice note with no caption
  truncated: boolean;         // the stored body was longer than we serve
  group_chat_id: string | null;
  group_title: string | null;
};

export type PersonMessagesPage = {
  total: number;                                       // under current filters
  channels: { channel: string; count: number }[];      // ignoring the channel filter
...
export type PersonDetail = {
  ...
  location: string | null;        // stored, editable; seeded from LinkedIn
...
export const api = {
  listPersons: (
    params: { q?: string; company_id?: string; circle?: string; source?: string; sort?: PersonSort; limit?: number; offset?: number },
  ...
  listUnknownAttendees: (limit = 25, opts?: { cookieHeader?: string }) =>
    jsonFetch<UnknownAttendee[]>(`/api/meetings/unknown-attendees?limit=${limit}`,
      { cookieHeader: opts?.cookieHeader }),

  dismissUnknownAttendee: (email: string) =>
    jsonFetch<void>("/api/meetings/unknown-attendees/dismiss",
      { method: "POST", body: JSON.stringify({ email }) }),

  linkUnknownAttendee: (body: { email: string; person_id?: string; name?: string | null }) =>
    jsonFetch<{ person_id: string; linked: number }>("/api/meetings/unknown-attendees/link",
  ...
  personMessages: (
    ...
    return jsonFetch<PersonMessagesPage>(
      `/api/persons/${personId}/messages?${sp}`, { cookieHeader: opts?.cookieHeader });
  },
  ...
  createFollowup: (body: { person_id: string; due_date?: string | null; due_time?: string | null; topic?: string | null; priority?: FollowupPriority }) =>
  ...
  // One request, not one per row: rescheduling 80 follow-ups as 80 PATCHes
  // takes long enough that the list visibly redraws mid-flight.
  bulkPatchFollowups: (
    ids: string[],
    body: { due_date?: string | null; due_time?: string | null; connected?: boolean; status?: string; priority?: FollowupPriority },
  ) =>
    jsonFetch<{ changed: number }>(`/api/followups/bulk`, {
      method: "PATCH", body: JSON.stringify({ ids, ...body }),
    }),
  bulkDeleteFollowups: (ids: string[]) =>
    jsonFetch<{ changed: number }>(`/api/followups/bulk/delete`, {
      method: "POST", body: JSON.stringify({ ids }),
    }),
  // Spread the overdue backlog across the coming days, capped per day. No body:
  // the server decides who moves where, atomically.
  distributeFollowups: () =>
    jsonFetch<{ moved: number; days: number }>(`/api/followups/distribute`, {
      method: "POST",
    }),
  // The day a new follow-up should default to: the next one under the per-day
  // cap, so a contact-page add doesn't pile onto an already-full tomorrow.
  nextFollowupSlot: (opts?: { cookieHeader?: string }) =>
    jsonFetch<{ date: string }>(`/api/followups/next-slot`, { cookieHeader: opts?.cookieHeader }),
  ...
  /** Upload the phone book exported from an iPhone (.vcf). Cards are stored
   *  at once; people appear after the normalizer's next run. */
  uploadIphoneContacts: async (file: globalThis.File) => {
    const fd = new FormData();
    fd.append("file", file);
    const res = await fetch(`/api/setup/iphone-contacts/upload`, { method: "POST", body: fd });
    if (!res.ok) {
    ...
    return res.json() as Promise<{
      ok: boolean;
      counts: {
        cards: number; new: number; updated: number; unchanged: number;
        with_phone: number; numbers: number; numbers_without_country: number;
      };
    }>;
  },
  ...
  // --- streams (deal tags) ---
  listStreams: (opts?: { cookieHeader?: string }) =>
    jsonFetch<StreamRow[]>(`/api/streams`, { cookieHeader: opts?.cookieHeader }),
  createStream: (name: string) =>
    jsonFetch<StreamRow>(`/api/streams`, { method: "POST", body: JSON.stringify({ name }) }),
  renameStream: (name: string, next: string) =>
    jsonFetch<StreamRow>(`/api/streams/${encodeURIComponent(name)}`,
      { method: "PATCH", body: JSON.stringify({ name: next }) }),
  /** `force` also strips the tag from deals still carrying it. */
  deleteStream: (name: string, opts?: { force?: boolean }) =>
    jsonFetch<void>(`/api/streams/${encodeURIComponent(name)}${opts?.force ? "?force=true" : ""}`,
      { method: "DELETE" }),
  ...
  countPersons: (params: { q?: string; company_id?: string; circle?: string; source?: string }, opts?: { cookieHeader?: string }) => {
    const sp = new URLSearchParams();
    if (params.q) sp.set("q", params.q);
  ...
  /** Identity sources that actually exist, with people counts — the options
   *  for the People list's source filter. */
  listPersonSources: (opts?: { cookieHeader?: string }) =>
    jsonFetch<PersonSource[]>("/api/persons/sources", { cookieHeader: opts?.cookieHeader }),

  listUnplacedNumbers: (opts?: { cookieHeader?: string }) =>
    jsonFetch<{ numbers: UnplacedNumber[]; total: number; cards: number }>(
      "/api/contacts/numbers/unplaced", { cookieHeader: opts?.cookieHeader }),
  fixNumbers: (items: { digits: string; action: "set" | "ignore"; international?: string }[]) =>
    jsonFetch<{ fixed: number; ignored: number; cards: number; joined: number }>(
      "/api/contacts/numbers/fix", { method: "POST", body: JSON.stringify({ items }) }),
  ...
  /** Stop offering one candidate name for this person. Server-side, because
   *  the suggestions are re-derived from the raw tables on every request. */
  dismissNameSuggestion: (personId: string, name: string) =>
    jsonFetch<void>(
      `/api/persons/${personId}/name-suggestions/dismiss`,
  ...
  setPersonLocation: (personId: string, location: string | null) =>
    jsonFetch<{ ok: true; location: string | null }>(
      `/api/persons/${personId}/location`,
...
/** An email seen on your calls that isn't a contact yet. Grouped by address,
 *  so one unknown person across six meetings is a single row. */
export type UnknownAttendee = {
  email: string;
  name: string | null;
  meetings: number;
