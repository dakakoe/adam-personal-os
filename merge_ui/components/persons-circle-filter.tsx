 ...
 *
 *  `carry` is every other filter on the page: picking or clearing a circle
 *  keeps them, instead of dropping you back on an unfiltered list. */
export function PersonsCircleFilter({
  currentKey, currentLabel, carry,
}: {
  ...
  if (currentKey) {
    return (
      <span className="inline-flex items-center gap-1 h-8 px-2.5 rounded-md border border-border bg-accent/40 text-xs">
        ...
        <button
          type="button"
          onClick={() => router.push(personsHref(carry, { circle: null }))}
            ...
            <ul className="max-h-72 overflow-y-auto divide-y divide-border">
              ...
              <li>
                <button
                  ...
                  onClick={() => {
                    setOpen(false);
                    router.push(personsHref(carry, { circle: "__none__" }));
                  }}
              ...
              {circles.map((c) => (
                <li key={c.id}>
                  <button
                    ...
                    onClick={() => {
                      setOpen(false);
                      router.push(personsHref(carry, { circle: c.key }));
                    }}
                    className="w-full flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-accent"
                  >
