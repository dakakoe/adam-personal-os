...
import { defaultDueDate, dueLabel, channelLabel, daysLate } from "@/lib/followups";
...
export function FollowupsClient() {
  ...
  // follow-ups proper.
  type Feed =
    | { kind: "followup"; key: string; urgency: number; f: FollowupRow }
    | { kind: "cadence"; key: string; urgency: number; c: CircleDueRow };

  const feed: Feed[] = [
    ...(rows ?? []).map((f): Feed => ({
      kind: "followup", key: `f:${f.id}`, f,
      // Settled ones sink to the bottom; they're a record, not a debt.
      urgency: f.connected || f.status === "cancelled"
        ? Number.NEGATIVE_INFINITY
        : daysLate(f.due_date, f.due_time),
    })),
  ...
  async function create(v: { due_date: string; due_time: string | null; topic: string | null }) {
    if (!person) return;
    try {
