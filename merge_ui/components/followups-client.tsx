...
import { CalendarClock, Check, Pencil, Trash2, Undo2, MessageSquare, AlertCircle, Plus, ListChecks, CalendarRange } from "lucide-react";
import { api, type FollowupRow, type PersonRow, type CircleDueRow } from "@/lib/api";
import { defaultDueDate, dueLabel, channelLabel, daysLate, groupOf, compareGroups, groupDropTarget, type Group } from "@/lib/followups";
...
export function FollowupsClient() {
  ...
  // follow-ups proper.
  type Feed =
    | { kind: "followup"; key: string; urgency: number; group: Group; f: FollowupRow }
    | { kind: "cadence"; key: string; urgency: number; group: Group; c: CircleDueRow };
  ...
  /** Spread the overdue backlog across the coming days, capped at 10/day. The
   *  server does the assignment in one transaction; we just reload. */
      ...
      <div className="flex items-center gap-1 text-xs">
        ...
        {scope === "open" && overdueCount > 0 && (
          <button
            ...
            title={`Spread ${overdueCount} overdue across the coming days, up to 10 a day`}
