...
export function PersonRowActions({
  ...
  async function followUp() {
    setBusy("followup");
    try {
      // Land on the next day that still has room, not blindly on tomorrow —
      // the same capacity rule the Distribute button and the add forms use.
      // Falls back to tomorrow if the lookup fails, so it never blocks. No
      // topic yet — this is the decision to have a conversation, not the agenda.
      let due = defaultDueDate();
      try { due = (await api.nextFollowupSlot()).date; } catch { /* keep tomorrow */ }
      await api.createFollowup({ person_id: personId, due_date: due });
      const when = due === defaultDueDate()
        ? "tomorrow"
        : (() => { const [y, m, d] = due.split("-").map(Number);
                   return new Date(y, m - 1, d).toLocaleDateString([], { weekday: "short", month: "short", day: "numeric" }); })();
      toast.success(`Follow-up with ${displayName} planned for ${when}`, {
