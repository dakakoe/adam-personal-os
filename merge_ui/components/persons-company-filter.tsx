 ...
 *  `carry` is every other filter on the page, so choosing a company narrows
 *  the list you are looking at rather than replacing it. */
export function PersonsCompanyFilter({
  currentId, currentName, carry,
}: {
  ...
  if (currentId) {
    return (
      <span className="inline-flex items-center gap-1 h-8 px-2.5 rounded-md border border-border bg-accent/40 text-xs">
        ...
        <button
          type="button"
          onClick={() => router.push(personsHref(carry, { company: null }))}
  ...
  return (
    <CompanyPicker
      align="right"
      onPick={(c) => router.push(personsHref(carry, { company: c.id }))}
      trigger={
