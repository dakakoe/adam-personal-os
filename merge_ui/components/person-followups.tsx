...
import { api, type FollowupRow, type FollowupPriority } from "@/lib/api";
...
import { PriorityMark } from "@/components/followup-priority";
...
export function PersonFollowups({
  ...
  // The date a new follow-up defaults to: the next day still under the per-day
  // cap, fetched when you open the add form so a contact-page plan doesn't land
  // on an already-full day. Falls back to tomorrow if the lookup fails.
  const [slotDate, setSlotDate] = useState<string>(defaultDueDate());
      ...
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <button
          ...
          onClick={async () => {
            ...
            try {
              const { date } = await api.nextFollowupSlot();
              setSlotDate(date);
            } catch { setSlotDate(defaultDueDate()); }
            setAdding(true);
          }}
            ...
            return (
              <li key={f.id} className="flex items-start gap-2.5 px-3 py-2 group">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {/* A mark, not a control: the panel is a record of what you
                      owe this one person, and the triage happens on the
                      Follow-ups page where you can compare them. */}
                  {!f.connected && <span className="ml-1.5"><PriorityMark value={f.priority} /></span>}
                  {f.topic && (
