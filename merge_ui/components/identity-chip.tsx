...
import { CHANNEL_ICONS as ICONS, CHANNEL_COLORS as CHANNEL_COLOR } from "@/lib/channels";
...
function valueDisplay(source: string, value: string): { label: string; href: string | null } {
  ...
  switch (source) {
    ...
    case "whatsapp":
      // Stored as bare international digits (or 'lid:N' when WhatsApp hides
      // the number). Show it like a phone number; link only when it is one.
      if (v.startsWith("lid:")) return { label: "hidden number", href: null };
      return { label: `+${v.replace(/^\+/, "")}`, href: `https://wa.me/${v.replace(/^\+/, "")}` };
    case "phone":
      return { label: v.startsWith("+") ? v : `+${v}`, href: `tel:${v.startsWith("+") ? v : `+${v}`}` };
    case "email":
      return { label: v, href: `mailto:${v}` };
    case "iphone_contact":
      // The value is the card's UID — meaningless to a person.
      return { label: "phone book", href: null };
    case "website":
