...
export function OpportunitiesBoard({
  ...
  return (
    <>
      {/* Trello logic: every column is the SAME fixed width at every breakpoint
          and the board scrolls sideways. Previously they flexed to fill at lg+,
          which meant a card's width depended on how many stages you had — add a
          stage and every card everywhere got narrower, and titles that used to
          fit started wrapping to four lines. A fixed column is a stable place
          to read a card. */}
      <div className="flex items-start gap-3 pb-2 overflow-x-auto">
        ...
        {[...live, ...terminal].map((sc) => {
          ...
          return (
            <div
              ...
              className={cn(
                // 17rem ≈ Trello's 272px list: wide enough that a deal title
                // reads on two lines instead of four.
                "w-[17rem] shrink-0 rounded-lg border bg-card/30 p-2",
    ...
    >
      ...
      {(o.tags?.length ?? 0) > 0 && (
        <div className="flex flex-wrap gap-1 mt-1">
          {o.tags.map((t) => (
            // Was 9px muted-on-muted, which read as decoration. A stream is
            // how you find a deal on a crowded board, so it gets a filled chip
            // and foreground text — legible at a glance, still quieter than
            // the title.
            <span key={t} className="text-[10px] font-medium uppercase tracking-wide px-1.5 py-0.5 rounded-full border border-border bg-secondary/70 text-foreground/80">
              {t}
            </span>
          ))}
        </div>
