...
export function NameSuggestionBanner({
  ...
  /** Identities for hint-derivation (e.g. instagram handle → "Morrissey"). */
  identities?: IdentityRow[];
  ...
  useEffect(() => {
    ...
    api.nameSuggestions(personId)
      .then((r) => {
        ...
        // instead of typing from scratch. The case this is for: name is
        // "Telegram user N", no LinkedIn or email to mine, but an Instagram
        // handle was just added — its title-cased form is the obvious draft.
  ...
  /** Say no to one candidate for good. The suggestions are re-derived from
   *  the raw tables on every load, so declining has to be recorded server-side
   *  — dropping it from local state alone would bring it straight back. The
   *  local drop is just so the row leaves immediately. */
  async function decline(name: string) {
    setBusy(true);
    try {
      await api.dismissNameSuggestion(personId, name);
      setData((prev) => prev && {
        ...prev,
        suggestions: prev.suggestions.filter((s) => s.suggested !== name),
      });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Couldn't dismiss that name");
    } finally {
      setBusy(false);
    }
  }
      ...
      {hasSuggestions && (
        <>
          <p className="text-xs text-muted-foreground -mt-1">
            ...
            Click to apply, or dismiss one to stop being offered it.
          </p>
            ...
            {data.suggestions.map((s, idx) => (
              ...
              >
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => decline(s.suggested)}
                  disabled={busy}
                  title={`Never suggest “${s.suggested}” for this contact`}
                  aria-label={`Dismiss the name ${s.suggested}`}
