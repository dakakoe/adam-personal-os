...
import { PRIORITY_ORDER, PRIORITY_META } from "@/components/followup-priority";
...
export function FollowupBulkBar({
  count, onSetDate, onSetPriority, onConnect, onDelete, onClear, busy,
}: {
  count: number;
  onSetDate: (due_date: string | null, due_time: string | null) => void;
  onSetPriority: (priority: FollowupPriority) => void;
  onConnect: () => void;
  onDelete: () => void;
  onClear: () => void;
  busy: boolean;
  ...
  return (
    <div className="sticky bottom-3 z-20 mx-auto w-full max-w-3xl">
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card px-3 py-2 shadow-lg shadow-black/40">
        ...
        {/* Weighting is the one edit that's genuinely BETTER in bulk: you
            decide "this whole overdue pile is low" in one judgement, which is
            what makes the rest of the backlog land where it should. */}
        {PRIORITY_ORDER.map((p) => (
          <button
            key={p} type="button" onClick={() => onSetPriority(p)} disabled={busy}
            title={`Mark the selected follow-ups ${PRIORITY_META[p].label.toLowerCase()} priority`}
            className={cn("inline-flex items-center gap-1 h-8 px-2.5 rounded-md border text-sm disabled:opacity-50",
