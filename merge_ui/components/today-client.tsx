...
function ScheduleSection({ events, tz }: { events: CalendarEvent[]; tz?: string }) {
  ...
  const header = (
    <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-2 flex items-center gap-1.5">
      ...
      {/* NOT "live": this reads raw.gcal_event, which a timer refreshes every
          few minutes. Claiming live made a normal sync lag look like a bug —
          you move an event in Google, the panel disagrees, and the label says
          it shouldn't. "synced" sets the right expectation. */}
      <span className="font-normal normal-case text-[11px]">· synced from Google Calendar</span>
