...
import { PrioritySelect } from "@/components/followup-priority";
...
export function FollowupEditor({
  initialDate, initialTime, initialTopic, initialPriority, submitLabel, onSubmit, onCancel, autoFocusTopic,
}: {
  initialDate: string | null;
  initialTime?: string | null;
  initialTopic?: string | null;
  initialPriority?: FollowupPriority | null;
  submitLabel: string;
  onSubmit: (v: {
    due_date: string | null; due_time: string | null;
    topic: string | null; priority: FollowupPriority;
  }) => Promise<void>;
  onCancel?: () => void;
  ...
  // 'mid' unless you say otherwise: most follow-ups are ordinary, and a form
  // that made you choose would turn every one of them into a decision.
  const [priority, setPriority] = useState<FollowupPriority>(initialPriority ?? "mid");
  ...
  return (
    <div className="flex items-end gap-2 flex-wrap rounded-lg border border-border bg-card/40 p-2.5">
      ...
      <div>
        ...
        <PrioritySelect value={priority} onChange={setPriority} className="h-8 flex items-center" />
      </div>
