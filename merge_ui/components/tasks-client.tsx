...
import { groupOf, compareGroups, groupDropTarget } from "@/lib/day-groups";
...
/** Dot colour for a due-date group, keeping the old bucket palette: overdue is
 *  alarming, today/tomorrow warm, further days cool, settled done. */
function DOT_FOR_KEY(key: string): string {
  if (key === "overdue") return "bg-rose-500";
  if (key === "today") return "bg-amber-500";
  if (key === "tomorrow") return "bg-sky-500";
  if (key === "settled") return "bg-emerald-500";
  if (key === "someday") return "bg-muted-foreground/60";
  return "bg-sky-500/50";   // a specific future day
}
...
export function TasksClient({
  ...
  // Drag-to-reschedule (due-date grouping only): which task is in flight and
  // which day group it's hovering.
  ...
  // Grouping. In due mode a group carries `key` so it can be a drop target and
  // sorted by day; project mode has no key (dropping a task on a project would
  // be a reassign, which the detail panel handles, not this list).
  const groups: { key?: string; label: string; dot?: string; color?: string | null; items: TaskRow[] }[] = [];
  if (group === "due") {
    // Overdue / Today(date) / Tomorrow(date), then one group per day — the same
    // scheme the follow-ups list uses. "This week" was a 7-day bucket that, on a
    // weekend, swallowed most of next week; a date can't be wrong about its day.
    const map = new Map<string, { key: string; label: string; items: TaskRow[] }>();
    for (const t of tasks) {
      const g = groupOf(t.due_date, t.due_time, t.status === "done" || t.status === "cancelled");
      (map.get(g.key) ?? map.set(g.key, { key: g.key, label: g.label, items: [] }).get(g.key)!)
        .items.push(t);
    }
    for (const g of [...map.values()].sort((a, b) => compareGroups(a.key, b.key))) {
      groups.push(g);
    }
  } else {
  ...
  /** Drop a task onto a day group → re-date it to that day (time preserved).
   *  No date clears the date; Overdue/Settled reject the drop. */
  async function moveTask(id: string, key: string) {
    const target = groupDropTarget(key);
    if (!target.droppable) return;
    const t = tasks.find((x) => x.id === id);
    if (!t || (target.date ?? null) === (t.due_date ?? null)) return;
    try {
      await api.patchTask(id, { due_date: target.date });   // due_time left as-is
      refresh();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Couldn't move that task");
    }
  }
  ...
  return (
    <>
      ...
      {groups.length === 0 ? (
        ...
        const drop = g.key ? groupDropTarget(g.key) : { droppable: false, date: null };
        const dragActive = dragId !== null && drop.droppable;
        const dot = g.key ? DOT_FOR_KEY(g.key) : g.dot;
        return (
        <div
          key={g.key ?? gi}
          onDragOver={(e) => {
            ...
            e.preventDefault();
            if (dragOverKey !== g.key) setDragOverKey(g.key ?? null);
          }}
          ...
          onDrop={(e) => {
            ...
            setDragOverKey(null);
            if (id && g.key) void moveTask(id, g.key);
          }}
          className={cn("mb-4 rounded-lg transition-colors",
          ...
          <ul className="divide-y divide-border rounded-md border border-border overflow-hidden bg-card/40">
            {g.items.map((t) => (
              <TaskRowItem
                ...
                onOpen={() => setOpenTaskId(t.id)} onDelete={() => onDelete(t)} />
            ))}
          </ul>
        </div>
        );
...
function TaskRowItem({ t, onOpen, onDelete, draggable, onDragStart, onDragEnd, dragging }: {
  t: TaskRow; onOpen: () => void; onDelete: () => void;
  ...
  return (
    <li
      draggable={draggable}
      onDragStart={(e) => { onDragStart?.(); e.dataTransfer.setData("text/plain", t.id); e.dataTransfer.effectAllowed = "move"; }}
      onDragEnd={() => onDragEnd?.()}
      className={cn("flex items-center gap-3 px-3 sm:px-4 py-2.5 hover:bg-accent/20 transition-colors",
