 ...
 *  picking a source KEEPS an active search, company or circle instead of
...
export function PersonsSourceFilter({
  ...
  current: string | null;
  carry: { q?: string; company?: string; circle?: string };
}) {
  ...
  const go = (source: string | null) => {
    const params = new URLSearchParams();
    if (carry.q) params.set("q", carry.q);
    if (carry.company) params.set("company", carry.company);
    if (carry.circle) params.set("circle", carry.circle);
    if (source) params.set("source", source);
    // No offset: a changed filter starts at the first page, or you land on an
    // empty page 4 of a list that now has two pages.
    const qs = params.toString();
    router.push(`/persons${qs ? `?${qs}` : ""}`);
  };
