...
import { UnknownAttendees } from "@/components/unknown-attendees";
import { api, type CalendarEvent, type TaskRow, type ProjectRow, type TodayCounts, type Stats, type UnknownAttendee } from "@/lib/api";
...
async function fetchData() {
  ...
  let unknownAttendees: UnknownAttendee[] = [];
  try { counts = await api.getTodayCounts(opts); } catch { /* ignore */ }
  // Secondary panel: an empty list on failure, so a dashboard never breaks
  // because meetings haven't been normalized yet.
  try { unknownAttendees = await api.listUnknownAttendees(10, opts); } catch { /* ignore */ }
  ...
  return { counts, stats, events, eventsTz, tasks, projects, unknownAttendees };
}

export default async function TodayPage() {
  const { counts, stats, events, eventsTz, tasks, projects, unknownAttendees } = await fetchData();
  ...
  return (
    <AppShell>
      <div className="p-4 sm:p-6 mx-auto w-full max-w-6xl">
        ...
        {unknownAttendees.length > 0 && <UnknownAttendees initial={unknownAttendees} />}
