...
function IphoneContactsStep(_: StepProps) {
  const [busy, setBusy] = useState(false);
  const [result, setResult] = useState<string | null>(null);
  ...
  async function upload() {
    const f = fileRef.current?.files?.[0];
    if (!f) { toast.error("Pick the .vcf file exported from Contacts first"); return; }
    setBusy(true);
    try {
      const { counts: c } = await api.uploadIphoneContacts(f);
      const text = `Stored ${c.cards} contacts (${c.new} new, ${c.updated} updated). ` +
        "People appear within about 5 minutes.";
      setResult(text);
      setWarning(c.numbers_without_country > 0
        ? `${c.numbers_without_country} of ${c.numbers} numbers have no country code, so they can't be ` +
          "matched to anyone yet."
        : null);
      toast.success(text);
    } catch (e) { toast.error(e instanceof Error ? e.message : "Upload failed"); }
    setBusy(false);
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Import your phone book. Your own names for people come with it — including people
        WhatsApp only shows as a number. A contact whose number or email matches someone already
        here is added to them; a number shared by several people goes to your merge queue.
      </p>
      <ol className="text-sm text-muted-foreground list-decimal pl-5 space-y-1">
        <li>On the iPhone: <span className="text-foreground">Contacts → Lists</span>, press and hold
          {" "}<span className="text-foreground">All Contacts</span>, then <span className="text-foreground">Export</span> and save the file.</li>
        <li>Or on a computer: <span className="text-foreground">icloud.com/contacts</span>, select all,
          then <span className="text-foreground">Export vCard</span>.</li>
        <li>Upload the <code className="text-xs">.vcf</code> file here. Upload a newer export any time — only changed contacts are redone.</li>
      </ol>
      <div className="flex items-center gap-2">
        <input ref={fileRef} type="file" accept=".vcf,text/vcard,text/x-vcard"
          className="text-sm file:mr-2 file:rounded-md file:border file:border-border file:bg-secondary/40 file:px-2.5 file:py-1 file:text-xs file:text-foreground text-muted-foreground" />
        <Button size="sm" onClick={upload} disabled={busy}>
          {busy ? <Loader2 className="h-3.5 w-3.5 mr-1.5 animate-spin" /> : <Upload className="h-3.5 w-3.5 mr-1.5" />}
          Upload &amp; import
      ...
      {warning && (
        <p className="text-xs text-amber-400">
          {warning}{" "}
          <a href="/cleanup/numbers" className="underline underline-offset-2">Fix or delete them</a>
        </p>
      )}
    </div>
  );
...
const STEPS: Step[] = [
  ...
  { key: "iphone", title: "iPhone contacts", blurb: "Phone book import", sourceKey: null, Body: IphoneContactsStep },
