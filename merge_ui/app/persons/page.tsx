...
async function fetchData(
  ...
  circle: string | undefined, source: string | undefined, offset: number,
) {
  ...
  try {
    ...
    const [rows, count] = await Promise.all([
      api.listPersons({ q, company_id: companyId, circle, source, limit: PAGE_SIZE, offset }, { cookieHeader: cookie }),
...
export default async function PersonsPage({
  ...
  searchParams: Promise<{ q?: string; offset?: string; company?: string; circle?: string; source?: string }>;
}) {
  const { q, offset: offsetStr, company, circle, source } = await searchParams;
  const offset = Math.max(0, parseInt(offsetStr ?? "0", 10) || 0);
  const { rows, total } = await fetchData(q, company, circle, source, offset);
      ...
      <div className="p-4 sm:p-6 mx-auto w-full max-w-5xl">
        <header className="mb-6 flex items-baseline justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2 shrink-0">
            ...
            <PersonsSourceFilter current={source ?? null} carry={{ q, company, circle }} />
            {isAdmin && (
              <>
                <Link
                  href="/merge"
