...
export default async function OpportunitiesPage({
  ...
  // Name the filtered project in the header, so the counts below clearly
  // describe one project and not the whole pipeline.
  const projectName = sp.project_id ? projects.find((p) => p.id === sp.project_id)?.name : undefined;

  return (
    <AppShell>
      {/* Full width, unlike the rest of the app: a board is a horizontal
          surface and every rem of margin is one fewer card you can see. The
          max-w-7xl that suits reading pages was leaving a third of a wide
          screen empty next to a board that scrolls. */}
      <div className="p-4 sm:p-6 w-full">
        <header className="mb-4 sm:mb-6">
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-1">
            {opps.length} total{projectName ? ` in ${projectName}` : ""}{sp.stage ? ` in stage "${sp.stage}"` : ""}. Custom stages: intro → conversations → MoU → contract → active (or lost).
          </p>
        </header>
        ...
        <OpportunitiesClient initialOpps={opps} projects={projects} currentStage={sp.stage} view={view}
          projectFilter={sp.project_id ?? null} projectId={sp.project_id} />
      </div>
    </AppShell>
  );
}
