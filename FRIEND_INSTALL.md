...
This repo is a self-hosted **Personal OS**: your mail, Telegram, WhatsApp,
contacts, meetings, tasks, deals, and budget on your own server — with local-LLM
...
| WhatsApp (optional) | live chat ingest | nothing to obtain — the server links as a companion device with a code you enter on your phone. **Read the risk note below first.** |
| Granola API key (optional) | meeting recaps | pasted later in the Setup wizard, not during install |

### About WhatsApp

WhatsApp publishes no API for a personal account. The only way in is the
multi-device protocol — the same one WhatsApp Web and Desktop speak —
through an unofficial library. That means:

- **It is against WhatsApp's terms, and accounts do get banned.** In practice
  bans overwhelmingly hit accounts that send automated or bulk messages, which
  is exactly what this does not do: the bridge is read-only, with no send path
  anywhere in it. The risk is low, not zero. It is your call, and it is opt-in
  — skip it and everything else works.
- **It uses one of your four linked-device slots.**
- **Only one instance may run against the session.** Two clients fighting over
  it drops messages for both.
- A newly linked device receives almost no history. Importing your archive is
  a separate step — see
...
- optional: WhatsApp — my phone number in WHATSAPP_PHONE (digits only, no '+');
  nothing else to obtain, and it is paired after install
...
9. WhatsApp, only if I asked for it: it is the one Node service besides the
   UI, so `npm ci --omit=dev --omit=optional` in /srv/memory/apps/whatsapp
   (never rsync node_modules or session/). Then, with the unit STOPPED,
   source the env and run `node src/index.js pair`; give me the 8-character
   code to enter on my phone under Settings → Linked Devices → Link a Device
   → Link with phone number instead. Start the unit once it says paired.
   Full runbook: fetchers/whatsapp/README.md.
...
- The telethon session file is auth-token-equivalent: 0600, owned by ops,
  never in git, never in world-readable backups. The WhatsApp session
  directory is exactly the same: anyone holding it can read every message the
  account receives. 0700, excluded from git AND from rsync.
- A migration that changes a UNIQUE key ships together with every writer whose
  ON CONFLICT names it. No test catches that mismatch — Postgres only
  complains at execution time, on a timer, in production.
...
- WhatsApp, if you enabled it, is paired from the server rather than the
  wizard: `fetchers/whatsapp/README.md`.
