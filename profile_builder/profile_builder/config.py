...
class Config:
    ...
    # Only bring role cards up to date — no profile building, no LLM calls.
    # For the one-off card backfill; every normal run ends with a card pass.
    cards_only: bool
...
def load() -> Config:
    return Config(
        ...
        cards_only=_bool_env("PROFILE_CARDS_ONLY", default=False),
    )
