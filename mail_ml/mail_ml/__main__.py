...
async def _classify(args) -> None:
    ...
    try:
        ...
        # Then re-check every existing personal verdict against the guards.
        flips = classify_mod.reguard_rows(await db.fetch_personal_for_guard(pool))
        g = await db.upsert_classes(pool, flips)
    finally:
        await pool.close()
    from collections import Counter
    dist = Counter(v["content_class"] for v in verdicts)
    moved = Counter(v["content_class"] for v in flips)
    logging.info("classified %d messages: %s; guards moved %d personal: %s",
                 n, dict(dist), g, dict(moved))
    print({"classified": n, "distribution": dict(dist), "reguarded": g,
           "reguard_distribution": dict(moved)})
