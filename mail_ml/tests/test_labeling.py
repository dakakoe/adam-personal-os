...
def test_prefixed_noreply_and_fees_are_role_senders():
    assert L._is_role_sender("school_noreply@school.example")
    assert L._is_role_sender("acme-no-reply@acme.example")
    assert L._is_role_sender("fees@school.example")
    # ...but a human whose name merely starts like one is not.
    assert not L._is_role_sender("noreplyguy@example.com")
    assert not L._is_role_sender("feesmith@example.com")


def test_stranger_at_volume_threshold_and_exemptions():
    n = L.STRANGER_MIN_SENDS
    assert L.is_stranger_at_volume(False, False, n)
    assert not L.is_stranger_at_volume(False, False, n - 1)
    assert not L.is_stranger_at_volume(False, True, n * 10)   # someone you know
    assert not L.is_stranger_at_volume(True, False, n * 10)   # your own address
    assert not L.is_stranger_at_volume(None, None, None)


def test_bulk_verdict_defaults_to_transactional():
    assert L.bulk_verdict("Build #42 failed", "see logs") == L.TRANSACTIONAL
    assert L.bulk_verdict("Weekly digest", "unsubscribe here") == L.NEWSLETTER
    assert L.bulk_verdict("Your order receipt", "unsubscribe") == L.TRANSACTIONAL
