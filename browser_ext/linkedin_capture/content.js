...
(() => {
  ...
  // The second line of a GROUPED employer: how long you've been there in
  // total, with no date range. Two shapes in the wild —
  //   "4 yrs 8 mos"          duration alone
  //   "Full-time · 2 mos"    employment type stated once for the whole
  //                          employer rather than per role
  // Only the first was recognised, so a profile using the second was read as
  // a simple entry: title and company came out as the two lines above the
  // date, which are the location and the role.
  // Callers MUST rule out a date line first — "Jul 2026 - Present · 2 mos"
  // ends in a duration too.
  const GROUP_DURATION_RE =
    /(?:^|·\s*)\d+\s*(?:yrs?|years?|mos?|months?)(?:\s+\d+\s*(?:mos?|months?))?\s*$/i;
  ...
  function parseHistory(lines, limit) {
    ...
    let groupHeader = null;   // the duration line, which may name the employment type
    let groupLocation = null; // stated once for the employer, above the roles

    for (let i = 0; i < lines.length && out.length < limit; i++) {
      const line = lines[i];

      // Checked BEFORE the date test on purpose, but only for lines that
      // aren't dates — a date range ending "· 2 mos" would otherwise look
      // like a group header and swallow the entry.
      if (!DATE_RE.test(line) && GROUP_DURATION_RE.test(line)) {
        // The line above the duration is the employer these roles belong to.
        groupCompany = i > 0 ? lines[i - 1] : null;
        groupHeader = line;
        // A grouped employer states its location ONCE, between the header and
        // the first role — after the date come skill tags, not a place. The
        // line right after the header is the location only when it isn't
        // itself the role, i.e. when it isn't the line just before the date.
        groupLocation = null;
        const after = lines[i + 1];
        let nd = lines.length;
        for (let j = i + 1; j < lines.length; j++) {
          if (DATE_RE.test(lines[j])) { nd = j; break; }
        }
        if (after && nd > i + 2 && after.length <= 60 && !/[.!?]/.test(after)) {
          groupLocation = after;
        }
        continue;
      }
      ...
      if (COMPANY_LINE_RE.test(prevLine)) { groupCompany = null; groupHeader = null; groupLocation = null; }
      ...
      // In a group the employment type sits on the header ("Full-time · 2 mos"),
      // not on the company line, which is a bare employer name.
      const employment = EMPLOYMENT_RE.exec(groupHeader || company || prevLine || "");
      ...
      out.push({
        ...
        // Grouped entries take the employer's location; their post-date
        // lines are skill tags and would otherwise be read as a place.
        location: groupCompany
          ? groupLocation
          : (tail[0] && tail[0].length <= 60 && !/[.!?]/.test(tail[0]) ? tail[0] : null),
        description:
