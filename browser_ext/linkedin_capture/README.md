...
| emails, phones, websites, twitter, birthday | LinkedIn's Contact-info page — **only** when you press "Pull contact info" |

Everything is editable in the popup before you save; a selector LinkedIn
renames costs you one blank field, not the capture.

"Pull contact info" is behind its own button because it isn't a passive read:
clicking LinkedIn's "Contact info" **navigates** the tab to
`/in/<vanity>/overlay/contact-info/` and the scraper navigates back with
`history.back()`. Verified on live profiles — a 1st-degree connection yields
the email and birthday; a 2nd-degree one yields nothing, because LinkedIn
shows nothing.
