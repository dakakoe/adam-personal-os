...
def fetch_at_startup(sql: str, *args: Any) -> list[asyncpg.Record]:
    """One query before the server's event loop exists, for data baked into
    tool descriptions at registration. Opens and closes its own connection;
    the pool is still created lazily on the server's loop."""
    assert _cfg is not None, "db.configure(cfg) must be called first"

    async def go() -> list[asyncpg.Record]:
        conn = await asyncpg.connect(_cfg.db_url, timeout=10)
        try:
